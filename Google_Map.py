...
# 设置您的API密钥
api_key = 'AIzaSyBbqSQLhElKvrKs00uaG6pP6acy_fq99ok'
...
class place_info():
    ...
    def find_place(self, place, type):
        # 配置Geocoding API请求参数
        geocode_params = {
            'address': place,
            'key': api_key
...
if __name__ == '__main__':
    demo_test = place_info()

    type_list = ['tourist_attraction', 'restaurant', 'park', 'shopping_mall',
                 'bakery', 'cafe', 'clothing_store', 'drugstore', 'university']

    type = type_list[2]  # 设置为可以选择的逻辑

    place_name = '香港浸会大学' #可以给用户填写

    demo_test.find_place(place_name,type)